"""
Agent for technical indicator analysis in high-frequency trading (HFT) context.
Uses LLM and toolkit to compute and interpret indicators like MACD, RSI, ROC, Stochastic, and Williams %R.
"""
from langchain_core.prompts import ChatPromptTemplate, MessagesPlaceholder
from langchain_core.messages import ToolMessage, AIMessage
from prompt_provider import PromptProvider
import json

def create_indicator_agent(llm, toolkit, config=None):
    """
    Create an indicator analysis agent node for HFT. The agent uses LLM and indicator tools to analyze OHLCV data.
    """
    # プロンプトプロバイダーを初期化
    provider = PromptProvider(config)

    def indicator_agent_node(state):
        # --- Tool definitions ---
        tools = [
            toolkit.compute_macd,
            toolkit.compute_rsi,
            toolkit.compute_roc,
            toolkit.compute_stoch,
            toolkit.compute_willr,
        ]
        time_frame = state['time_frame']

        # --- 設定ベースでプロンプトを取得（kline_dataなしで） ---
        system_prompt = provider.format_prompt(
            "indicator_agent",
            "system",
            time_frame=time_frame,
            kline_data="[KLINE_DATA_PLACEHOLDER]"  # プレースホルダーを使用
        )

        # --- System prompt for LLM ---
        prompt = ChatPromptTemplate.from_messages(
            [
                ("system", system_prompt),
                MessagesPlaceholder(variable_name="messages"),
            ]
        )

        chain = prompt | llm.bind_tools(tools)
        messages = state.get("messages", [])

        # If messages is empty, add a default message for technical indicator analysis
        if not messages:
            from langchain_core.messages import HumanMessage
            kline_data_str = json.dumps(state["kline_data"], indent=2)
            messages = [HumanMessage(content=f"Analyze the provided {time_frame} kline data and compute relevant technical indicators.\n\nKline Data:\n{kline_data_str}")]
        
        # --- Step 1: Ask for tool calls ---
        ai_response = chain.invoke({"messages": messages})
        messages.append(ai_response)

        # --- Step 2: Collect tool results ---
        if hasattr(ai_response, "tool_calls"):
            for call in ai_response.tool_calls:
                tool_name = call["name"]
                tool_args = call["args"]
                # Always provide kline_data
                import copy
                tool_args["kline_data"] = copy.deepcopy(state["kline_data"])
                # Lookup tool by name
                tool_fn = next(t for t in tools if t.name == tool_name)
                tool_result = tool_fn.invoke(tool_args)
                # Append result as ToolMessage
                messages.append(
                    ToolMessage(
                        tool_call_id=call["id"],
                        content=json.dumps(tool_result)
                    )
                )

        # --- Step 3: Re-run the chain with tool results ---
        final_response = chain.invoke({"messages": messages})

        return {
            "messages": messages + [final_response],
            "indicator_report": final_response.content,
        }

    return indicator_agent_node