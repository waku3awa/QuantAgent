...
def create_pattern_agent(tool_llm, graph_llm, toolkit, config=None):
    """
    ...
    def pattern_agent_node(state):
        ...
        # --- 設定ベースでプロンプトを取得 ---
        system_prompt = provider.get_prompt("pattern_agent", "system")
        ...
        # If messages is empty, add a default HumanMessage with kline_data
        if not messages:
            kline_data_str = json.dumps(state["kline_data"], indent=2)
            messages = [HumanMessage(content=f"Analyze the provided kline data for candlestick patterns in {time_frame} timeframe.\n\nKline Data:\n{kline_data_str}")]
        ...
        if pattern_image_b64:
            # 設定ベースで画像分析プロンプトを取得
            image_analysis_text = provider.format_prompt(
                "pattern_agent",
                "image_analysis",
                time_frame=time_frame,
                pattern_descriptions=pattern_text
            )
            image_system_prompt = provider.get_prompt("pattern_agent", "image_system")
