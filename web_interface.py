...
class WebTradingAnalyzer:
    ...
    def set_language(self, language: str) -> bool:
        """Set the language for the prompt provider."""
        try:
            self.prompt_provider.set_language(language)
            # Update trading graph with new language configuration
            config = DEFAULT_CONFIG.copy()
            config["output_language"] = language
            self.trading_graph = TradingGraph(config)
            return True
        except Exception as e:
    ...
    def get_current_language(self) -> str:
        ...
        return self.prompt_provider.language

# Initialize the analyzer
analyzer = WebTradingAnalyzer()

def get_session_language():
    """Get language from session, defaulting to Japanese."""
    return session.get('language', 'ja')

def set_session_language(language):
    """Set language in session and update analyzer."""
...
@app.before_request
def before_request():
    """Initialize language setting before each request."""
    if 'language' not in session:
        session['language'] = 'ja'  # Default to Japanese
    analyzer.set_language(session['language'])
...
def set_language():
    """API endpoint to set the interface language."""
    try:
        data = request.get_json()
        ...
        success = set_session_language(language)
        if success:
            return jsonify({
                "success": True,
                "language": language,
            ...
            return jsonify({"success": False, "error": "Failed to set language"})
...
def get_language():
    """API endpoint to get the current interface language."""
    try:
        current_language = get_session_language()
        return jsonify({
