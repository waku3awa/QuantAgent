...
class SetGraph:
    ...
    def set_graph(self):
        ...
        agent_nodes['indicator'] = create_indicator_agent(self.graph_llm, self.toolkit, self.config)
        tool_nodes['indicator'] = self.tool_nodes['indicator']

        # create nodes for pattern agent
        agent_nodes['pattern'] = create_pattern_agent(self.agent_llm, self.graph_llm, self.toolkit, self.config)
        tool_nodes['pattern'] = self.tool_nodes['pattern']

        # create nodes for trend agent
        agent_nodes['trend'] = create_trend_agent(self.agent_llm, self.graph_llm, self.toolkit, self.config)
        tool_nodes['trend'] = self.tool_nodes['trend']

        # create nodes for decision agent
        # decision_agent_node = create_final_trade_decider(self.agent_llm, self.config)
        decision_agent_node = create_final_trade_decider(self.graph_llm, self.config)
